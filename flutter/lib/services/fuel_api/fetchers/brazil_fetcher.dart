...
Future<List<Station>> fetchBrazil(
    double lat, double lng, double radiusKm, String fuelType) async {
  final workerUrl = Env.brazilWorkerUrl;
  if (workerUrl.isEmpty) return generateDemoStations(lat, lng, radiusKm, 'BR');

  try {
    final res = await _dio.get(
        '$workerUrl/api/brazil?lat=$lat&lng=$lng&radius=$radiusKm');
