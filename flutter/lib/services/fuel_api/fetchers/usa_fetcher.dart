...
Future<List<Station>> fetchUSA(
    double lat, double lng, double radiusKm, String fuelType) async {
  final workerUrl = Env.usaWorkerUrl;
  if (workerUrl.isEmpty) return generateDemoStations(lat, lng, radiusKm, 'US');

  try {
    final res = await _dio.get(
        '$workerUrl/api/usa?lat=$lat&lng=$lng&radius=$radiusKm');
